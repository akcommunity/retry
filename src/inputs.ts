...
export function getInputs(): Inputs {
  ...
  const command2 = getInput('command2', { required: false });
  const command_on_error = getInput('command_on_error', { required: true });
  const trigger_error_text = getInput('trigger_error_text', { required: true });  
